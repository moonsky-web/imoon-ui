...
export function colorRegister(colorClass, color, namespaced) {
  const clazz = `.${namespaced}.${colorClass}`, classNs = `${clazz}.${namespaced}`;
  return `${clazz}{background:${color};border-color:${color};color:#fff;}
    ...
    ${classNs}-auto:hover{box-shadow: 0 0 0 .1em ${hoverRgba(color)};}
    ${classNs}-auto:active{box-shadow: 0 0 0 .2em ${activeRgba(color)};}
    ${classNs}-outline,
    ${classNs}-dashed{color:${color};background:#fff;}
    ${classNs}-ghost{color:${color};background:transparent;}
    ${classNs}-outline:hover,
    ${classNs}-dashed:hover,
